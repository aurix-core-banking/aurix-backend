...
@TestConfiguration
public class CartoesTestConfig {
    ...
    @Bean
    @Primary
    public MlFraudClient mlFraudClient() {
        var client = mock(MlFraudClient.class);
        when(client.avaliar(any())).thenReturn(
            new MlFraudClient.FraudResponse("APROVADO", 0.95, "OK"));
        return client;
    }
}
